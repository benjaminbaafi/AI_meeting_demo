...
class BatchJobInfo(BaseSchema):
    """Information about a single job within a batch."""
    job_id: str = Field(..., description="Job identifier")
    filename: str = Field(..., description="Original filename")
...
class BatchStatusResponse(BaseSchema):
    ...
    processing: int = Field(default=0, ge=0, description="Number of jobs in progress")
    failed: int = Field(default=0, ge=0, description="Number of failed jobs")
    queued: int = Field(default=0, ge=0, description="Number of queued jobs")
    ...
    jobs: List[BatchJobInfo] = Field(default_factory=list, description="Individual job statuses")
    created_at: datetime = Field(..., description="Batch creation time")
    updated_at: datetime = Field(..., description="Last update time")
    estimated_completion_time: Optional[datetime] = Field(None)


class BatchResults(BaseSchema):
    """Aggregated results from a batch."""
    batch_id: str = Field(..., description="Batch identifier")
    total_files: int = Field(..., ge=1)
    successful: int = Field(..., ge=0, description="Number of successful jobs")
    failed: int = Field(..., ge=0, description="Number of failed jobs")
